...
export class PurchaseInvoiceListComponent implements OnInit, AfterViewInit, OnDestroy {
  displayedColumns: string[] = ['id', 'date', 'vendorRefNo', 'documentNo', 'contact', 'amount', 'actions'];
  ...
  private readonly documentType = 'PurchaseInvoice';
  ...
  private _createPdf(invoice: any, companyProfile: CompanyProfileDto): void {
    ...
    // 2. Info Block (Bill From & Invoice Details)
    let leftY = currentY;
    let rightY = currentY;

    // --- Left: Bill From Details ---
    doc.setFontSize(8).setFont("helvetica", "normal");
    doc.text("Bill From:", margin, leftY);
    leftY += 4;
    ...
    doc.text("PURCHASE INVOICE", rightX, rightY, { align: 'right' });
    rightY += 6;
    ...
    // 5. Save
    doc.save(`PurchaseInvoice_${invoice.documentNo || 'Doc'}.pdf`);
  }
