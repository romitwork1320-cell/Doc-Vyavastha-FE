...
@Component({
  ...
  template: `
    <div class="dialog-header p-24 d-flex align-items-center justify-content-between border-bottom" style="overflow-x: hidden;">
      <h4 class="mat-headline-6 m-0">{{ isEdit ? 'Edit Application Status' : 'Create Application Status' }}</h4>
      ...
      <form [formGroup]="form" class="crm-form">
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="text" formControlName="name" class="crm-input" placeholder="Ex: Pending" [attr.disabled]="isEdit && form.get('name')?.disabled ? true : null">
          </div>
          <div class="form-group">
            <label>Color Code <span class="required">*</span></label>
            <div class="color-picker-wrapper">
              <input type="color" formControlName="colorCode" class="color-input">
              <input type="text" formControlName="colorCode" class="crm-input" style="flex: 1; min-width: 0;" placeholder="#22c55e">
            </div>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" formControlName="displayOrder" class="crm-input">
          </div>
          <div class="form-group">
  ...
  styles: [`
    ...
    .color-input { -webkit-appearance: none; -moz-appearance: none; appearance: none; width: 40px; height: 40px; background-color: transparent; border: none; cursor: pointer; padding: 0; outline: none; flex-shrink: 0; }
    .color-input::-webkit-color-swatch { border-radius: 8px; border: 1px solid #cbd5e1; }
    .color-input::-moz-color-swatch { border-radius: 8px; border: 1px solid #cbd5e1; }
...
export class StatusDialogComponent {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: Partial<ApplicationStatus & { inUse?: boolean }>,
    private service: ApplicationStatusService,
    private snackBar: MatSnackBar
  ) {
    this.isEdit = !!data.id;
    this.form = this.fb.group({
      name: [{ value: data.name || '', disabled: this.isEdit && data.inUse }, [Validators.required, Validators.minLength(2), Validators.maxLength(100)]],
      colorCode: [data.colorCode || '#3b82f6', [Validators.required]],
      description: [data.description || ''],
      displayOrder: [data.displayOrder || 1, [Validators.required, Validators.min(1)]],
  ...
  save() {
    ...
    const request$ = this.isEdit 
      ...
      : this.service.create(payload as ApplicationStatus);
    ...
    request$.subscribe({
      ...
      error: (err) => {
        this.snackBar.open(err.error?.message || err.message || 'Error saving status', 'Close', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
        this.isSaving = false;
      }
    });
  }
