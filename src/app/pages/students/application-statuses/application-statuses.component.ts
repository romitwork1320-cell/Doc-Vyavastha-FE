...
export class ApplicationStatusesComponent implements OnInit, OnDestroy {
  statuses = signal<ApplicationStatus[]>([]);
  isLoading = false;
  searchQuery = '';
  filterStatus = '';
    ...
    this.service.getAll().subscribe({
      next: (res) => {
        if (res.success && res.data) {
          // Client-side sorting by displayOrder
          this.statuses.set(res.data.sort((a, b) => a.displayOrder - b.displayOrder));
        }
        this.isLoading = false;
      },
      error: () => {
        this.snackBar.open('Failed to load application statuses', 'Close', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
        this.isLoading = false;
      }
    });
  }
  ...
  get filteredStatuses() {
    return this.statuses().filter(s => {
      ...
      const matchSearch = !q || (s.name && s.name.toLowerCase().includes(q)) || (s.description && s.description.toLowerCase().includes(q));
      return matchStatus && matchSearch;
    });
  }

  count(status: string) { return this.statuses().filter(s => s.status === status).length; }

  openDialog(status?: ApplicationStatus): void {
  ...
  deleteItem(id: string): void {
    if (confirm('Are you sure you want to delete this status?')) {
      this.service.delete(id).subscribe({
        next: (res) => {
          if (res.success) {
            this.snackBar.open('Deleted successfully', 'OK', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
            this.loadData();
          } else {
            this.snackBar.open(res.message || 'Failed to delete', 'Close', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
          }
        },
        error: (err) => {
          const msg = err.error?.message || err.message || 'Error deleting status';
