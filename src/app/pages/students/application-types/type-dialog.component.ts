...
@Component({
  ...
  template: `
    <div class="dialog-header p-24 d-flex align-items-center justify-content-between border-bottom" style="overflow-x: hidden;">
      <h4 class="mat-headline-6 m-0">{{ isEdit ? 'Edit Application Type' : 'Create Application Type' }}</h4>
...
export class TypeDialogComponent {
  ...
  save() {
    ...
    // getRawValue so we include the disabled 'name' field in the payload
    const payload = { ...this.data, ...this.form.getRawValue() };

    const request$ = this.isEdit 
      ? this.service.update(payload.id!, payload)
      : this.service.create(payload as ApplicationType);
    ...
    request$.subscribe({
      ...
      error: (err) => {
        this.snackBar.open(`Error: ${err.message}`, 'Close', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
        this.isSaving = false;
      }
    });
  }
