...
export class ApplicationTypesComponent implements OnInit, OnDestroy {
  types = signal<ApplicationType[]>([]);
  isLoading = false;
  searchQuery = '';
  filterStatus = '';
  ...
  constructor(private service: ApplicationTypeService,
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: () => {
        this.snackBar.open('Failed to load application types', 'Close', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
        this.isLoading = false;
      }
    });
  }
  ...
  openDialog(type?: ApplicationType): void {
  ...
  deleteItem(id: string): void {
    if (confirm('Are you sure you want to delete this type?')) {
      this.service.delete(id).subscribe({
        ...
        error: (err) => {
          const msg = err.error?.message || err.message || 'Error deleting type';
