...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TablerIconsModule, MatDialogModule, MatButtonModule, MatSelectModule],
  template: `
    <div class="dialog-header p-24 d-flex align-items-center justify-content-between border-bottom" style="overflow-x: hidden;">
      <h4 class="mat-headline-6 m-0">{{ isEdit ? 'Edit Fee Type' : 'Create Fee Type' }}</h4>
    ...
    <mat-dialog-content class="mat-typography" style="padding-top: 16px; overflow-x: hidden;">
      <form [formGroup]="form" class="crm-form">
        <div class="form-group">
          ...
          <input type="text" formControlName="name" class="crm-input" placeholder="e.g. Registration Fee">
        </div>
        
        <div class="form-group">
          <label>Amount (₹) <span class="required">*</span></label>
          <input type="number" formControlName="amount" class="crm-input" placeholder="0.00">
        </div>
        ...
        <div class="form-group">
          ...
          <mat-select formControlName="categoryIds" multiple class="crm-input" style="padding: 0;">
            <mat-option *ngFor="let cat of categories()" [value]="cat.id" [disabled]="isCategoryDisabled(cat.id)">
              {{ cat.name }} <span *ngIf="isCategoryDisabled(cat.id)" style="font-size: 11px; color: #ef4444; margin-left: 8px;">(In Use)</span>
            </mat-option>
          </mat-select>
        </div>
...
export class FeeTypeDialogComponent implements OnInit {
  ...
  isCategoryDisabled(categoryId: string): boolean {
    if (!this.data.allFeeTypes) return false;
    for (const ft of this.data.allFeeTypes) {
      if (ft.id !== this.data.id && ft.categoryIds?.includes(categoryId)) {
        return true;
      }
    }
    return false;
