...
export class FormTypesComponent implements OnInit, OnDestroy {
  types = signal<FormType[]>([]);
  isLoading = false;
  searchQuery = '';
  filterStatus = '';
  ...
  constructor(private service: FormTypeService,
  ...
  loadData(): void {
    ...
    this.service.getAll().subscribe({
      ...
      error: () => {
        this.snackBar.open('Failed to load form types', 'Close', { duration: 3000, horizontalPosition: 'right', verticalPosition: 'top' });
        this.isLoading = false;
      }
    });
  }
  ...
  get filteredTypes() {
    return this.types().filter(t => {
      ...
      const matchSearch = !q || (t.name && t.name.toLowerCase().includes(q)) || (t.description && t.description.toLowerCase().includes(q));
      return matchStatus && matchSearch;
    });
  }

  count(status: string) { return this.types().filter(t => t.status === status).length; }

  openDialog(type?: FormType): void {
    const dialogRef = this.dialog.open(FormTypeDialogComponent, {
      width: '90vw',
      maxWidth: '600px',
      data: type ? { ...type } : {},
      disableClose: true
  ...
  deleteItem(id: string): void {
    if (confirm('Are you sure you want to delete this form type?')) {
      this.service.delete(id).subscribe({
        ...
        error: (err) => {
          const msg = err.error?.message || err.message || 'Error deleting form type';
