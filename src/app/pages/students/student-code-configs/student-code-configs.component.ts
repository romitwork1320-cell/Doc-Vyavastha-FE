...
import { CodeConfigDialogComponent } from './code-config-dialog.component';
import { FabClickService } from 'src/app/services/fab-click.service';
import { Subject, forkJoin } from 'rxjs';
import { takeUntil } from 'rxjs/operators';

@Component({
  selector: 'app-student-code-configs',
  standalone: true,
  imports: [CommonModule, FormsModule, TablerIconsModule, MatSlideToggleModule],
  templateUrl: './student-code-configs.component.html',
  styles: [`
...
export class StudentCodeConfigsComponent implements OnInit, OnDestroy {
  configs = signal<any[]>([]);
  isLoading = false;
  searchQuery = '';
  filterStatus = '';
  ...
  constructor(private service: StudentCodeConfigurationService,
  ...
  loadData(): void {
    ...
    forkJoin({
      ...
      next: ({ configs, categories }) => {
        if (configs.success && configs.data && categories.success && categories.data) {
          const cats = categories.data;
          const mappedData = configs.data.map(config => {
            const cat = cats.find(c => c.id === config.categoryId);
            return { ...config, categoryName: cat ? cat.name : 'Unknown' };
          });
          this.configs.set(mappedData);
        }
  ...
  get filteredConfigs() {
    return this.configs().filter(c => {
      const matchStatus = !this.filterStatus || String(c.isActive) === this.filterStatus;
      const q = this.searchQuery.toLowerCase();
      const matchSearch = !q || (c.categoryName && c.categoryName.toLowerCase().includes(q)) || (c.prefix && c.prefix.toLowerCase().includes(q));
      return matchStatus && matchSearch;
    });
  }

  count(status: string) { return this.configs().filter(c => String(c.isActive) === status).length; }


  openDialog(config?: any): void {
    const dialogRef = this.dialog.open(CodeConfigDialogComponent, {
      width: '90vw',
      maxWidth: '600px',
      data: config ? { ...config } : {},
      disableClose: true
  ...
  toggleStatus(conf: any, event: any): void {
    const newStatus = event.checked;
    const updatedConf = { ...conf, isActive: newStatus };
    this.service.update(conf.id, updatedConf).subscribe(res => {
      if (res.success) {
        conf.isActive = newStatus;
      } else {
